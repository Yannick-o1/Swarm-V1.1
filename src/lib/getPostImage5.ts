...
const INDEX_FILE = './lastIndex5.txt';  // Different index file for each bot

async function getNextImagePath(): Promise<string> {
    try {
        // Read all images from directory
        const files = await fs.readdir('./images5');  // Different directory for each bot
        ...
        return path.join('./images5', imageFiles[currentIndex]);
    } catch (error) {
...
export default async function getPostImage() {
    try {
        ...
        const postText = "Bot 5's custom message";  // Different message for each bot
        ...
        await agent.login({
            identifier: process.env.BSKY_HANDLE_5!,
