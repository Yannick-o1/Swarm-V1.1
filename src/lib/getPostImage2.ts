...
const INDEX_FILE = './lastIndex2.txt';  // Different index file for each bot

async function getNextImagePath(): Promise<string> {
    try {
        // Read all images from directory
        const files = await fs.readdir('./images2');  // Different directory for each bot
        ...
        return path.join('./images2', imageFiles[currentIndex]);
    } catch (error) {
...
export default async function getPostImage() {
    try {
        ...
        const postText = "Bot 2's custom message";  // Different message for each bot
        ...
        await agent.login({
            identifier: process.env.BSKY_HANDLE_2!,
